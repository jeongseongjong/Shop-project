 ...
 * 메소드와 메소드간에
 * web 브라우저와 컨트롤러(메소드)간에
 * 컨트롤러와 view.jsp 간에 데이터를 교환하는 매개체 역할을 수행
 * 일반적으로 select되는 table의 칼럼을 포함하고
 * web borwser에서 form tag 사용하여
 * 		컨트롤러로 데이터를 보낼 때
 ...
 * 캡슐화 : getter, setter 메소드의 코드 정의
 * 		예) b_date_time 필드변수에는 2020-02-26 14:06:00 형태의 문자열만 저장하도록
 * 			setter() method에 관련 코드를 정의 할 수 있다.
 * 			또는 
 * 			나이를 저장하는 필드변수 같으면 나이의 범위를 0~100까지
 * 			제한 하는등의 코드를 setter 메서드에 정의할 수 있다.
 * 
 * 			성적처리를 수행한다 라면
 * 			총점을 참조하는 getter method를 정의하고
 * 			여기에 과목의 총합계를 계산하는 코드를 추가한 후
 * 			결과 값을 return 하도록 만들 수 있다.
 * 
 * 			캡슐화(투명성)
 * 			총점 getter method의 내부에 어떤 코드가 있는지 몰라도
 * 			getter 메소드만 호출하면 총점을 알 수 있다.
 */

/*
 * mybatis @Alias()
 * mybatis 초기설정에서 typeAliasPackage를 설정하면
 * VO클래스의 이름을 참조하여 Alias를 만드는데
