...
public class BBsController {
	 ...
	 * BBsService interface를 impl한 클래스의 코드가 미완성 상태지만
	 * 		BBsController 입장에서는 BBsService*의 코드가 완성되어 있다.라는
	 * 		전제하에 Controller 코드 작성을 진행할 수 있다.
	 * 만약 이후에 BBsService*의 코드가 완성되고
	 * 		여러개의 BBsServiceImpl* 클래스가 작성되면
	 * 		필요한 클래스를 가져다가 부착만하면
	 * 		프로젝트가 완성이 될것이다.
	 * 
	 * 결합도를 낮추는 결과가 된다.
	 * 결합도 : 모듈간의 의존도, 모듈 코드작성의 유연성과 관련
	 * 결합도는 낮게, 응집도는 높은 모듈간 연계가 좋은 설계이다.
	 */
	@Autowired
	@Qualifier("bbsV2")
	 ...
	 * bbs_list.jps 와 rendering하도록 수행한다.
	 */
	 ...
	 * - 버튼이 2개 있다.
	 * - 그 중 저장(글쓰기) 버튼에는 아무런 타입도 지정하지 않았기 때문에
	 * 		<form> tag에 지정된 action path로 method에 지정된 방식으로 데이터를 전송한다.
	 * - 이러한 버튼의 기본 기능을 submit이라고 한다.
	 * 
	 * bbs_write.jsp에서는 <form> tag에 action을 제거했다.
	 * - 이렇게 되면 처음 게시판 작성 화면을 열기 위해서 사용했던
	 * - path인 ${rootPath}/bbs/insert 가 
	 * 		action에 자동으로 설정이 된다.
	...
	public String delete(@RequestParam("b_id")long id) {
