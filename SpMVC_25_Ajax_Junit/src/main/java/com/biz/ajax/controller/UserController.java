...
public class UserController {
	 ...
	 * Sp3점대에서는 기본으로 설정을 해주어야만 제대로 VO에서 데이터를 받는다.
	 * Sp4에서 부터는 선택사항으로 설정되고 VO에 Annotation을 붙이지 않아도
	 * form에서 전송되는 데이터를 잘 받았다.
	 * 
	 * form의 input에서 name으로 설정된 이름의 변수가 
	 * VO에 있으면 알아서 잘 매칭되어 데이터를 받을 수 있었다.
	 * 혹시 VO에 있는 변수 값이 전송되지 않더라도 문제가 없고 
	 * VO에 없는 변수값이 전송되더라도 오류가 나거나 하지 않았다.
	 * 
	 * @ModelAttribute를 사용하는 또 하나의 이유는
	 * form에서 받은 데이터를 다시 view로 재전송할때
	 ...
	 * return문에서 view 파일을 찾지 않고
	 * return 문의 데이터를 있는 그대로 전송하는 역할을 수행한다.
	 */
	@ResponseBody
