...
public class PageService {
	...
	private long listPerPage = 10; // 리스트 버튼 개수
	private long pageCount = 5; // 한 화면에 보여질 페이지 개수
	...
	/*
	 * 최소한의 조건으로 페이지를 계산하기 위한 method 전체 데이터 개수와 현재 선택된 페이지 번호만 매개변수로 받아서 페이지 정보를
	 * 만들어내기
	 */
	public PageDTO makePagination(long totalCount, long currentPageNo) {
		...
		// 마지막 쪽번호 = (전체데이터 + (한 페이지의 리스트 개수 -1)) / 한페이지의 리스트 개수
		long finalPageNo = (totalCount + (listPerPage - 1)) / listPerPage;

		// naver는 page를 검색할 때 1000페이지가 넘어가면 오류를 낸다.
//		finalPageNo = finalPageNo > 1000 ? 1000 : finalPageNo; // 3항 연산자
//		if (finalPageNo > 1000)
//			finalPageNo = 1000; // if문을 이용한 연산
		...
		// 하단에 리스트로 보여줄 페이지 계산
		// cur : 3이면 1 ~ 5, cur 10이면 8 ~ 12까지 이런식으로 보여라
		long startPageNo = ((currentPageNo - 1) / this.pageCount) * this.pageCount + 1;
		//(currentPageNo = 3이라가정시 currentPageNo와 startPageNo는 둘다 3이된다 왜 연산하는가?)
		...
		if(isNowFinal)
			pageDTO.setNextPageNo(finalPageNo);
		else pageDTO.setNextPageNo
